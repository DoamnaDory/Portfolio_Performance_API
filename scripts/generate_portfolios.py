...
from app.infrastructure.repositories import PortfolioRepository, TransactionRepository
...
async def create_random_transactions(
    ...
    for i in range(num_transactions):
        ...
        company_name = PriceService.TICKER_TO_COMPANY.get(ticker)  # Получаем имя для логирования
        if not company_name:
        ...
        if transaction_type == "sell":
            ...
            if held_qty < quantity:
                logger.debug(f"Недостаточно {ticker} ({company_name}) для продажи. Имеется: {held_qty}, запрошено: {quantity}. Пропускаем.")
                continue
        ...
        logger.debug(f"Добавлена транзакция: {transaction_type} {quantity}x {ticker} ({company_name}) @ {buy_price:.2f}")
...
async def generate_portfolios(
    ...
    async with AsyncSession(engine) as db:
        ...
        if clear_existing:
            ...
            logger.warning("Очистка портфелей и транзакций НЕ РЕАЛИЗОВАНА в этом скрипте. "
        ...
        for i in range(num_portfolios):
            ...
            logger.info(f"Создан портфель: {portfolio.name} (ID: {portfolio.id})")

            logger.info(f"Создание {transactions_per_portfolio} транзакций для портфеля {portfolio.id}...")
            await create_random_transactions(db, portfolio.id, tickers, transactions_per_portfolio)
...
def main():
    parser = argparse.ArgumentParser(description="Генератор тестовых портфелей и транзакций.")
    parser.add_argument("--portfolios", type=int, default=NUM_PORTFOLIOS, help="Количество портфелей для создания (по умолчанию: 5)")
    parser.add_argument("--transactions", type=int, default=TRANSACTIONS_PER_PORTFOLIO, help="Количество транзакций на портфель (по умолчанию: 3)")
    parser.add_argument("--clear", action='store_true', help="Очистить существующие портфели перед созданием новых (НЕ РЕАЛИЗОВАНО)")

    args = parser.parse_args()

    logger.info(f"Запуск генератора с параметрами: портфелей={args.portfolios}, транзакций={args.transactions}, очистка={args.clear}")

    asyncio.run(generate_portfolios(args.portfolios, args.transactions, args.clear))
